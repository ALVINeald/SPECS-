...
// ✅ FIX 2: Guard against empty array before calling max()
$worstSingleTotal = !empty($singleStoreTotals)
    ? max(array_column($singleStoreTotals, 'total'))
    : 0;

$totalSaving = ($bestSingleStore && $bestSingleStore['total'] > $splitTotal)
    ? $bestSingleStore['total'] - $splitTotal
    : 0;
$worstSaving = $worstSingleTotal > $splitTotal
    ? $worstSingleTotal - $splitTotal
    : 0;
...
/* ✅ FIX 1: Chevron starts pointing UP (▼ rotated 180°) when body is open */
.stop-chevron { font-size:.7rem; color:var(--muted); margin-left:4px; transition:transform .2s; display:inline-block; }
...
<div class="ctr">
  ...
  <div class="savings-hero">
    ...
    <div style="background:rgba(255,255,255,.1);border-radius:var(--r);padding:12px 16px;font-size:.78rem;color:rgba(255,255,255,.8);max-width:260px">
      💡 <strong style="color:var(--gold)">How it works:</strong> SPECS has checked every item in your basket and assigned each one to the store selling it cheapest. Your route visits all those stores in order.
    </div>
  </div>
    ...
    <div>
      <div class="map-card">
        <div class="map-header">
          ...
          <?php if ($gmapsUrl): ?>
          <a href="<?= htmlspecialchars($gmapsUrl) ?>" target="_blank" rel="noopener" class="gmaps-btn">
        ...
        <div class="stops-wrap">
          ...
          <div class="stop-card" id="stopcard-<?= $sid ?>">
            <div class="stop-header" onclick="toggleStop(<?= $sid ?>)">
              ...
              <div class="stop-right">
                ...
                <div class="stop-items-count">
                  <?= count($stop['items']) ?> item<?= count($stop['items']) > 1 ? 's' : '' ?> to buy
                  <?php /* ✅ FIX 1: chevron starts with class "open" because body starts open */ ?>
                  <span class="stop-chevron open" id="chev-<?= $sid ?>">▼</span>
                </div>
              </div>
            </div>
    ...
    <div class="route-sidebar">
      ...
      <div class="assign-card">
        ...
        foreach ($splitStops as $sid => $st) {
            $storeColorMap[$sid] = $stopColors[$ci % count($stopColors)];
            $ci++;
        }
        ...
        <div class="assign-row">
          <div class="assign-product">
            ...
            <div class="assign-qty"><?= htmlspecialchars($item['unit']) ?> × <?= $item['quantity'] ?></div>
          </div>
      ...
      <div class="compare-card">
        ...
        <div class="compare-highlight">
          ...
          <div style="font-size:.8rem;color:var(--leaf);font-weight:700"><?= $stopCount ?> store<?= $stopCount > 1 ? 's' : '' ?></div>
        </div>
...
splitStopsJS.forEach((stop, i) => {
  ...
  const icon = L.divIcon({
    html: `<div style="background:${color};color:${textColor};width:36px;height:36px;border-radius:50%;
                display:flex;align-items:center;justify-content:center;
                font-family:'Nunito',sans-serif;font-weight:900;font-size:15px;
                border:3px solid #fff;box-shadow:0 3px 12px rgba(0,0,0,.4)">${i + 1}</div>`,
  ...
  const itemList = stop.items.map(it =>
    `<div style="padding:4px 0;border-bottom:1px solid #eee;font-size:.78rem">
      <b>${it.name}</b> × ${it.qty}&nbsp;
      <span style="color:#2d6a4f;font-weight:700">UGX ${Number(it.line_total).toLocaleString()}</span>
     </div>`
  ).join('');
  ...
  L.marker([c.lat, c.lng], { icon }).addTo(map)
    .bindPopup(`
      <div style="font-family:'Nunito Sans',sans-serif;min-width:200px">
        <div style="background:${color};color:${textColor};padding:8px 12px;margin:-8px -12px 10px;
                    border-radius:6px 6px 0 0;font-family:'Nunito',sans-serif;font-weight:900">
          Stop ${i + 1}: ${c.name}
        </div>
        <div style="font-size:.74rem;color:#888;margin-bottom:8px">
          ${c.address} · Buy ${stop.items.length} item${stop.items.length > 1 ? 's' : ''}
        </div>
        ${itemList}
        <div style="padding:8px 0 0;font-family:'Nunito',sans-serif;font-weight:900;font-size:.9rem;color:#18382a">
          Subtotal: UGX ${Number(stop.subtotal).toLocaleString()}
        </div>
      </div>
    `, { maxWidth: 240 });
...
if (latLngs.length > 1) {
  L.polyline(latLngs, { color:'#52b788', weight:4, opacity:.85, dashArray:'10,6' }).addTo(map);
...
// ── TOGGLE STOP BODY ─────────────────────────────────────────
// ✅ FIX 1: Both body and chevron toggled together correctly
