...
<div class="ctr">
    ...
    <div style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;margin-bottom:5px">
        <span style="font-weight:700;font-size:.88rem">
          ...
          <?php if ($isBest): ?><span style="background:#d4edda;color:#155724;font-size:.65rem;font-weight:800;padding:2px 7px;border-radius:99px;margin-left:6px">BEST</span><?php endif; ?>
        </span>
        <span style="font-family:'Nunito',sans-serif;font-weight:900;color:<?= $isBest?'var(--leaf)':'var(--ink)' ?>"><?= formatPrice($cp['price']) ?></span>
      </div>
    ...
    new Chart(document.getElementById('trendChart'), {
      ...
      options: {
        ...
        scales: {
          ...
          y: { title: { display: true, text: 'Price (UGX)' },
               ticks: { callback: v => 'UGX ' + v.toLocaleString() } }
        },
        plugins: {
          tooltip: { callbacks: { label: ctx => ctx.dataset.label + ': UGX ' + ctx.parsed.y.toLocaleString() } }
        }
      }
    });
  })();
  ...
  <div class="card">
    <div class="empty-state">
      <div class="ei">📈</div>
      <p>No price history recorded yet for this product.<br><small style="color:var(--muted)">History is recorded each time an admin updates a price.</small></p>
    </div>
  </div>
  <?php endif; ?>
